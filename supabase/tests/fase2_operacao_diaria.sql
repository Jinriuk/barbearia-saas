...
-- A mensalidade precisa vir COM a transação vinculada, que é como
-- sell_customer_membership grava. `get_client_insights` soma o gasto do
-- assinante pela transação (e não por mp.amount) de propósito: cobrança
-- estornada não pode contar como gasto. Sem o vínculo, a fixture media um
-- caminho que não existe em produção e o assinante saía com R$ 0,00.
with paid as (
  insert into public.financial_transactions
    (barbershop_id, type, status, category, description, amount, paid_at, payment_method)
  values
    ('b0000000-0000-4000-8000-00000000000a', 'income', 'paid', 'membership',
     'Clube Op — mensalidade', 100, now() - interval '10 day', 'pix')
  returning id
)
insert into public.membership_payments
  (barbershop_id, membership_id, amount, payment_method, period_start, period_end,
   transaction_id)
select 'b0000000-0000-4000-8000-00000000000a', 'b7000000-0000-4000-8000-000000000001',
   100, 'pix', now() - interval '10 day', now() + interval '20 day', paid.id
from paid;
