...
-- INSUFFICIENT_STOCK (exige a migration 202607080016 aplicada).
insert into public.appointment_products (id, barbershop_id, appointment_id, product_id, quantity, unit_price, status)
values ('e5000000-0000-4000-8000-000000000002', 'e0000000-0000-4000-8000-00000000000b',
        'e3000000-0000-4000-8000-000000000002', 'e4000000-0000-4000-8000-000000000001', 99, 30, 'pending')
...
begin
  begin
    set local role authenticated;
    set local request.jwt.claims to '{"sub":"f0000000-0000-4000-8000-000000000002","role":"authenticated"}';
    -- usa a mesma reserva se o on conflict impediu a segunda linha
    perform public.confirm_product_sale(ap.id)
    from public.appointment_products ap
    where ap.product_id = 'e4000000-0000-4000-8000-000000000001' and ap.quantity = 99;
