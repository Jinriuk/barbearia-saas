 ...
 */
export function ProfitCalculator({ planCents }: { planCents: number }) {
  ...
  return (
    <div className="rounded-[2rem] border border-white/10 bg-white/[.03] p-7 sm:p-8">
      ...
      <div className="mt-7 rounded-2xl border border-emerald-500/20 bg-emerald-500/[.07] p-6">
        <p className="text-sm text-stone-400">
          Recuperando <strong className="text-stone-200">5%</strong> da base que
          parou de aparecer — {recovered}{" "}
          {recovered === 1 ? "cliente" : "clientes"} por mês:
        </p>
        <p className="mt-2 font-mono text-4xl font-semibold text-emerald-400">
          +{formatPriceBRL(gainCents)}
          <span className="ml-2 align-middle font-sans text-sm font-normal text-stone-500">
            por mês
          </span>
        </p>
      ...
      <Link
        href="/cadastro"
        className="btn-shine mt-6 inline-flex h-12 items-center justify-center rounded-full bg-amber-500 px-7 text-[15px] font-semibold text-stone-950 transition-colors hover:bg-amber-400"
      >
...
function Field({
  ...
  return (
    <div>
      <label
        htmlFor={id}
        className="flex items-baseline justify-between text-sm text-stone-400"
      >
        {label}
        <output htmlFor={id} className="font-mono text-base text-stone-100">
          {render(value)}
        </output>
      </label>
      <input
        ...
        className="mt-3 h-2 w-full cursor-pointer appearance-none rounded-full bg-white/10 accent-amber-500"
      />
    </div>
  );
}
